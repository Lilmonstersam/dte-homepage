...
2. **Retire Niubo site-wide.** Niubo is no longer part of DTE's range (client, July 2026). Remove Niubo from the live nav and brand pages, and 301 `/product-category/niubo-mulchers/` and its subcategories to the closest Virnig equivalents (drum/disc mulchers) or `/products/`. The mock-up's "Forestry Mulchers" pathway now points at `/product-category/virnig/skid-steer-drum-mulchers/` — confirm that category is populated and indexed, since "forestry mulcher" remains a headline build-target keyword.
...
14. **Taxonomy sprawl.** Product categories overlap three ways (brand: greentec/virnig, plus the retiring niubo tree; market: orchards/forestry/agriculture/municipality + carrier subcats; plus `/vegetation-management/`, `/other/`, legacy `/orchards/mowers/`, `/orchards/mulchers/`, `/orchards/canopy-management/`). Agree the canonical browse taxonomy and noindex/301 the rest so the new nav doesn't split ranking signals.
